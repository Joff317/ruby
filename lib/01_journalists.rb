...
def number_handle(handle)
  puts "il y à #{handle.length} dans cette array"
end
  puts number_handle(handle)


#Quel est le handle le plus court de cette liste ?
def handle_min(handle)
  puts "Le plus court est #{handle.min_by(&:length)}"
end

puts handle_min(handle)

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)

def handle_5_caracter(handle)
   print "le nombre de handle avec 5 caractère est "
   puts handle.count{ |element| element.length == 6 }
end

puts handle_5_caracter(handle)
...
def upperstr(handle)
  puts "Il y à"
  handle.count{|element| element[1].match?( /[[:upper:]]/ )}
end


puts upperstr(handle)


#Trie la liste de handle par ordre alphabétique.

def handle_alphabet(handle)
  puts "voici l'ordre alph"
  handle.sort_by(&:downcase)
end

puts handle_alphabet(handle)

#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)

def handle_size(handle)
  puts "voici la taille"
  handle.sort_by!(&:length) 
end

puts handle_size(handle)


#Quelle est la position dans l'array de la personne @epenser ?

def find_value(handle)
  puts " l'élément est en position #{handle.find_index("@epenser")}"
end
puts find_value(handle)
...
def handle_repartition(handle)
 repartition = handle.group_by(&:length)
 tabvalue =[]
 ...
 hash = tabkey.zip(tabvalue).to_h 
   (hash.length).times do |i|
    puts " il y à #{hash.values[i]} handle qui a #{hash.keys[i]-1} caractère"
   end
  
end

 handle_repartition(handle)
